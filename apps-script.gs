...
const TOKEN = "my-name-is-piseth-7979-is-a-string";
...
function handleRequest(e) {
  try {
    ...
    // Parse payload from query/form parameter
    const payload = params.payload ? JSON.parse(params.payload) : undefined;
    return jsonResponse({ data: handleAction(action, payload) });
  } catch (err) {
    ...
    case "updateProfile": {
      ...
      const existingRow = rows.find((r) => r[0] === "data");
        ...
        if (existingRow && existingRow[1]) {
          try { existing = JSON.parse(existingRow[1]); } catch { existing = {}; }
        }
